...
const noticeResponse = await fetch(`https://raw.githubusercontent.com/openai/codex/rust-v${version}/NOTICE`);
if (!noticeResponse.ok) throw new Error('Could not obtain the Codex third-party notices.');
await writeFile(join(output, 'NOTICE-Codex.txt'), await noticeResponse.text());
